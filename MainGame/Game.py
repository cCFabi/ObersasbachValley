...
def load_character_images(direction):
    images = []
    for i in range(4):
        path = os.path.join('character', direction, f'{i}.png')
        try:
...
background_tmx = load_pygame("UI/GAMEMEM.tmx")
...
popup_background = pygame.image.load("other UI/Menu.png").convert_alpha()
...
class MarketField:
    def __init__(self, x, y):
        ...
        self.rect = self.image.get_rect(topleft=(x, y))
        self.above_image = pygame.image.load("other UI/SELL.png").convert_alpha()
...
def add_field(fields, mouse_x, mouse_y, character, field_cost):
    ...
    new_field = Field(grid_x, grid_y, "field/field.png")
...
def main():
    ...
    field_preview_image = pygame.image.load("field/field.png").convert_alpha()
